...
def launch_distributed_script(tune_script):
  ...
  if NUM_GPU_WORKERS == 1:
    text_file = open("fine_tune_src/distributed_peft_scripts/common/accelerate_configs/accelerate_single_config.yaml.tmpl")
  elif NUM_GPU_WORKERS > 1:
    text_file = open("fine_tune_src/distributed_peft_scripts/common/accelerate_configs/accelerate_multi_config.yaml.tmpl")
  else:
    sys.exit(
